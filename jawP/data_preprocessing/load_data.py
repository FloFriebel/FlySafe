...
def load_data(_start, _end):
    ...
    data_list = []
    city_codes = {"Zurich": 'zh', "Lugano": 'lu', "Innsbruck": 'in', "Bolzano": 'bo'}
    for city, response in zip(("Zurich", "Lugano", "Innsbruck", "Bolzano"), responses):
   # """ city_query = [0, 1, 2, 3]
   # print(city_query) """
    #for city in city_query:
        #response = responses[city] #append function to an empty list
        ...
        data_list.append(pd.DataFrame(data).set_index("date").add_suffix(f"_{city_code}"))

    correct = pd.concat(data_list, axis=1)
    return correct   # should return df 840 X 20 for _start & _end "2024-02-10", "2024-03-15"
# Example
if __name__=="__main__":
    data_new = load_data("2024-02-10", "2024-03-15")
    print(data_new)
